import { ArrayLayoutProps, RankedTester } from '@jsonforms/core';
import React from 'react';
export declare const MaterialListWithDetailRenderer: ({ uischemas, schema, uischema, path, errors, visible, label, required, removeItems, addItem, data, renderers, config }: ArrayLayoutProps) => JSX.Element;
export declare const materialListWithDetailTester: RankedTester;
declare const _default: React.ComponentType<import("../../../core/lib").OwnPropsOfControl>;
export default _default;
